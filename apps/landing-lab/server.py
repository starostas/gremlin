...
def serve(args):
        ...
        def do_POST(self):
            ...
            try:
                ...
                if (not isinstance(settings, dict)
                        ...
                        or settings['preset'] not in ('lander',)
                        or type(settings['cases']) is not int or settings['cases'] not in (2048, 8192)
    ...
    print(f'Landing Lab: http://{args.host}:{args.port}', flush=True)
    try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--engine', type=Path, default=ROOT / 'engine/target/release/landing-lab')
    ...
    parser.add_argument('--remote-engine', default='/root/gremlin-validation/apps/landing-lab/engine/target/release/landing-lab')

...
def serve(args):
    ...
    def job(settings):
        ...
        try:
            ...
            for line in process.stdout:
                ...
                event(value)
                completed = ((settings['mode'] == 'both'
                              and value.get('kind') == 'comparison' and value.get('matching') is True)
                             or (settings['mode'] != 'both' and value.get('kind') == 'done'
                                 and value.get('mode') == settings['mode']))
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            files = {'/': ('index.html', 'text/html; charset=utf-8'),
                     ...
                     '/robot.js': ('robot.js', 'text/javascript; charset=utf-8'),
        ...
        def do_POST(self):
            ...
            try:
                ...
                if (not isinstance(settings, dict)
                        ...
                        or settings['mode'] not in ('gpu',)
                        or settings['preset'] not in ('robot',)
                        or type(settings['cases']) is not int or settings['cases'] not in (128, 512)
    ...
    print(f'Tiny Robot: http://{args.host}:{args.port}', flush=True)
    try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--engine', type=Path, default=ROOT / 'engine/target/release/tiny-robot')
    ...
    parser.add_argument('--remote-engine', default='/root/gremlin-validation/apps/tiny-robot/engine/target/release/tiny-robot')

...
class ServerTest(unittest.TestCase):
    def test_replay_only_and_request_boundaries(self):
        ...
        with tempfile.TemporaryDirectory(prefix='robot-server-test-') as temp:
            ...
            try:
                ...
                self.assertTrue(all(e['solved'] == e['cases'] for e in sample['events'] if e['kind'] == 'done'))
                valid = {'mode': 'gpu', 'preset': 'robot', 'cases': 128, 'seed': 1}
    ...
    def test_final_event_releases_a_lingering_process(self):
        with socket.socket() as sock:
            sock.bind(('127.0.0.1', 0))
            port = sock.getsockname()[1]
        with tempfile.TemporaryDirectory(prefix='robot-final-test-') as temp:
            ...
            try:
                for _ in range(100):
                    try:
                        ...
                        time.sleep(.03)
                settings = {'mode': 'gpu', 'preset': 'robot', 'cases': 128, 'seed': 1}
                request = urllib.request.Request(base + '/api/run', data=json.dumps(settings).encode(),
                                                 headers={'Content-Type': 'application/json'})

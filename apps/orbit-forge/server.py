...
import threading
import urllib.parse
from benchmark import benchmark, AccuracyFailure
...
def serve(args):
        ...
        try:
            ...
            if args.ssh:
                command = ['ssh', '-o', 'BatchMode=yes', '-o', 'ConnectTimeout=10',
                           ...
                           shlex.join(['timeout', '180', args.remote_engine])]
            else:
            ...
            if cancelled:
                stop()
            watchdog = threading.Timer(240, stop)
            watchdog.start()
            ...
            for line in process.stdout:
                ...
                if value.get('kind') == 'done':
                    event({'kind':'stage','message':'Compiling the winner and checking native runtime'})
                    if value.get('passed'):
                        try:
                            value['benchmark'] = benchmark(value)
                        except AccuracyFailure as error:
                            value['passed'] = False
                            value['failures'] += 1
                            value['checked'] += 1
                            value['max_error'] = max(value['max_error'], error.error)
                            value['benchmark'] = {'available':False,'reason':str(error),'counterexample':{'m':error.m,'e':error.e,'error':error.error}}
                        except Exception as error:
                            value['benchmark'] = {'available':False,'reason':str(error)}
                    value.pop('llvm', None)
                event(value)
        ...
        def do_POST(self):
            ...
            try:
                ...
                if self.path != '/api/run':
                    return self.send(404, {'error': 'Not found'})
                if (not isinstance(settings, dict) or set(settings) != {'seed','tolerance'}
                        or type(settings['seed']) is not int or settings['seed'] not in (1,2,3)
                        or type(settings['tolerance']) not in (int,float) or settings['tolerance'] not in (0.001,0.0001,0.00001)):
                    raise ValueError('Expected seed 1–3 and a supported angular tolerance')
    ...
    print(f'Orbit Forge: http://{args.host}:{args.port}', flush=True)
    try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--engine', type=Path, default=ROOT / 'engine/target/release/orbit-forge')
    ...
    parser.add_argument('--remote-engine', default='/root/gremlin-validation/apps/orbit-forge/engine/target/release/orbit-forge')

    ...
    def test_replay_only_and_request_boundaries(self):
        ...
        with tempfile.TemporaryDirectory(prefix='sculptor-server-test-') as temp:
            ...
            try:
                ...
                self.assertTrue(all(e['passed'] for e in sample['events'] if e['kind'] == 'done'))
                valid = {'tolerance':0.0001,'seed':1}
                for settings in [valid, dict(valid, tolerance=True), dict(valid, seed=True), dict(valid, tolerance=-1), [], {}]:
            ...
            try:
                for _ in range(100):
                    try:
                        ...
                        time.sleep(.03)
                settings = {'tolerance':0.0001,'seed':1}
                request = urllib.request.Request(base + '/api/run', data=json.dumps(settings).encode(),
                                                 headers={'Content-Type': 'application/json'})
